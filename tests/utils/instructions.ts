...
export async function initializeRandomPool(
  ...
  const [auth] = await getAuthAddress(cpSwapProgram);
  const randomPool = Keypair.generate()
  const poolAddress = randomPool.publicKey;
  const [lpMintAddress] = await getPoolLpMintAddress(
    poolAddress,
    cpSwapProgram
  );
  ...
  const tx = await program.methods
    .proxyInitializeRandomPool(initAmount.initAmount0, initAmount.initAmount1, new BN(0))
    .accounts({
      cpSwapProgram: cpSwapProgram,
      creator: creator.publicKey,
      ammConfig: configAddress,
